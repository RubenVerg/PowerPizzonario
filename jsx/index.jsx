...
export async function renderList(container, toc, url) {
	...
	toc.appendChild(<div class='w3-row table-of-contents'>
		{[...'#abcdefghijklmnopqrstuvwxyz'].map(l => <a class='w3-col s2 m1' {...(Object.keys(letters).includes(l) ? { href: `#letter-${l}` } : { disabled: 'disabled' })}>{l.toUpperCase()}</a>)}
	</div>);
